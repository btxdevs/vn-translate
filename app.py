...
class VisualNovelTranslatorApp:
            ...
            if not frame_available:
                if not self.capturing:
                    ...
                    if frame is not None:
                        ...
                        self.take_snapshot()
                    if not self.using_snapshot:
                        messagebox.showwarning("Warning", "Could not capture frame.", parent=self.master)
                        return
                else:
                    messagebox.showwarning("Warning", "Waiting for first frame.", parent=self.master)
                    return
    ...
    def start_snip_mode(self):
        ...
        with OCR_ENGINE_LOCK:
            if not self.ocr:
                messagebox.showwarning("OCR Not Ready", "OCR engine not initialized.", parent=self.master)
                return
    ...
    def display_snip_translation(self, text, region):
        ...
        try:
            ...
            self.current_snip_window = ClosableFloatingOverlayWindow(self.master, roi_name=SNIP_ROI_NAME, initial_config=snip_config, manager_ref=None)
    ...
    def _display_frame(self, frame):
        ...
        try:
            ...
            self.canvas.create_image(self.frame_display_coords["x"], self.frame_display_coords["y"], anchor=tk.NW, image=self.display_frame_tk, tags=("display_content", "frame_image"))
            self._draw_rois()
        except Exception as e:
    ...
    def _draw_rois(self):
        ...
        for i, roi in enumerate(self.rois):
            ...
            try:
                ...
                self.canvas.create_text(dx1 + 3, dy1 + 1, text=roi.name, fill="lime", anchor=tk.NW, font=("TkDefaultFont", 8), tags=("display_content", "roi_drawing", f"roi_label_{i}"))
            except Exception as e:
    ...
    def on_mouse_up(self, event):
        ...
        if abs(x2d - x1d) < min_size or abs(y2d - y1d) < min_size:
            messagebox.showwarning("ROI Too Small", f"Min {min_size}x{min_size} px required.", parent=self.master)
            return
        ...
        if not roi_name:
            i = 1
            roi_name = f"roi_{i}"
            while roi_name in [r.name for r in self.rois if r.name != SNIP_ROI_NAME]:
                i += 1
                roi_name = f"roi_{i}"
        elif roi_name in [r.name for r in self.rois if r.name != SNIP_ROI_NAME]:
            ...
            messagebox.showerror("Invalid Name", f"Cannot use reserved name '{SNIP_ROI_NAME}'.", parent=self.master)
            return
        ...
        if abs(ox2 - ox1) < 1 or abs(oy2 - oy1) < 1:
            messagebox.showwarning("ROI Too Small", "Calculated ROI size too small.", parent=self.master)
            return
