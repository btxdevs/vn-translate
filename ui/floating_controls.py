...
class FloatingControls(tk.Toplevel):
    def __init__(self, master, app_ref):
        ...
        self.retranslate_btn = ttk.Button(
            button_frame, text="🔄", width=3, style="Floating.TButton",
        ...
        self.force_retranslate_btn = ttk.Button(
            button_frame, text="⚡", width=3, style="Floating.TButton",
        ...
        self.snip_btn = ttk.Button(
            button_frame, text="✂️", width=3, style="Floating.TButton",
        ...
        initial_auto_state = (
            self.app.translation_tab.is_auto_translate_enabled()
            if hasattr(self.app, 'translation_tab') and self.app.translation_tab else False
        )
        self.auto_var = tk.BooleanVar(value=initial_auto_state)
        self.auto_btn = ttk.Checkbutton(
            button_frame, text="🤖", style="Toolbutton.TCheckbutton",
            variable=self.auto_var, command=self.toggle_auto_translate, width=3
        )
        ...
        initial_overlay_state = (
            self.app.overlay_manager.global_overlays_enabled
            if hasattr(self.app, 'overlay_manager') and self.app.overlay_manager else True
        )
        self.overlay_var = tk.BooleanVar(value=initial_overlay_state)
        self.overlay_btn = ttk.Checkbutton(
            button_frame, text="👁️", style="Toolbutton.TCheckbutton",
            variable=self.overlay_var, command=self.toggle_overlays, width=3
        )
        ...
        self.close_btn = ttk.Button(
            button_frame, text="✕", width=2, style="Floating.TButton",
    ...
    def copy_last_translation(self):
        ...
        if last_result and isinstance(last_result, dict):
            copy_text = ""
            for roi in getattr(self.app, 'rois', []):
                roi_name = roi.name
    ...
    def update_button_states(self):
        try:
            if hasattr(self.app, 'translation_tab') and self.app.translation_tab and hasattr(self, 'auto_var'):
                if self.auto_btn.winfo_exists():
                    self.auto_var.set(self.app.translation_tab.is_auto_translate_enabled())
            if hasattr(self.app, 'overlay_manager') and self.app.overlay_manager and hasattr(self, 'overlay_var'):
                if self.overlay_btn.winfo_exists():
                    self.overlay_var.set(self.app.overlay_manager.global_overlays_enabled)
        except tk.TclError:
            print("Warning: TclError during floating controls state update.")
        except Exception as e:
