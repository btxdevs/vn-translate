...
class ROITab(BaseTab):
    def setup_ui(self):
        ...
        color_btn_frame.grid(row=4, column=0, columnspan=4, pady=(10, 0))
    ...
    def pick_color(self, color_type):
        ...
        try:
            color_code = colorchooser.askcolor(title=title, initialcolor=initial_color_hex, parent=self.app.master)
    ...
    def show_filtered_preview(self):
        roi = self.get_selected_roi_object()
        if roi and not self.color_widgets['enabled_var'].get():
    ...
    def _show_preview(self, filtered=False):
        ...
        if self.app.using_snapshot and self.app.snapshot_frame is not None:
            ...
            source_frame = capture_window(self.app.selected_hwnd)
    ...
    def move_roi_down(self):
        ...
        try:
            ...
            print(f"Error moving ROI down: {e}")

    def delete_selected_roi(self):
        roi = self.get_selected_roi_object()
        if not roi or roi.name == SNIP_ROI_NAME:
            return
        if not messagebox.askyesno("Delete ROI", f"Delete ROI '{roi.name}'?", parent=self.app.master):
            return
        ...
        if roi.name in self.app.text_history:
            del self.app.text_history[roi.name]
        if roi.name in self.app.stable_texts:
            del self.app.stable_texts[roi.name]
        self.update_roi_list()
