...
class TranslationTab(BaseTab):
    def setup_ui(self):
        ...
        preset_index = 0
        if last_preset_name in self.preset_names:
    ...
    def save_basic_settings(self, event=None):
        ...
        if settings_to_update and update_settings(settings_to_update):
    ...
    def on_preset_selected(self, event=None):
        ...
        try:
            ...
            print("Error updating preset UI elements.")
    ...
    def get_current_preset_values_for_saving(self):
        try:
            ...
            if not preset_data["api_url"] or not preset_data["model"]:
                print("Warning: Saving preset with empty API URL or Model.")
            return preset_data
        except ValueError as e:
            messagebox.showerror("Error", f"Invalid number format in Preset Details: {e}", parent=self.app.master)
            return None
    ...
    def save_preset(self):
        ...
        if preset_data is None:
            return
        if not messagebox.askyesno("Confirm Save", f"Overwrite preset '{preset_name}' with current settings?", parent=self.app.master):
            return
    ...
    def save_preset_as(self):
        ...
        if new_name in self.translation_presets:
            if not messagebox.askyesno("Overwrite", f"Preset '{new_name}' already exists. Overwrite?", parent=self.app.master):
                return
    ...
    def delete_preset(self):
        ...
        if not preset_name:
            ...
            return
        if not messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete preset '{preset_name}'?", parent=self.app.master):
            return
        if preset_name in self.translation_presets:
            ...
            if save_translation_presets(self.translation_presets):
                self.preset_names = sorted(list(self.translation_presets.keys()))
                self.preset_combo['values'] = self.preset_names
                new_selection = self.preset_names[0] if self.preset_names else ""
            ...
            try:
                ...
                if "error" in translated_segments:
                    ...
                    if hasattr(self.app, 'overlay_manager'):
                        ...
                        if first_roi:
                            self.app.master.after_idle(lambda: self.app.overlay_manager.update_overlay(first_roi, "Error!"))
                    ...
                    self.app.master.after_idle(lambda: self.update_translation_results(translated_segments, preview_text))
            except Exception as e:
